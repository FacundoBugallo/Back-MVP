...
USER_ID = 1  # ⚠️ Temporal, en el futuro se sacará del token del usuario autenticado
...
def update_income(income_id: int, updates: IncomeUpdate, db: Session = Depends(get_db)):
    income = crud.income.get_income(db=db, income_id=income_id, user_id=USER_ID)
    if not income:
        raise HTTPException(status_code=404, detail="Ingreso no encontrado")
